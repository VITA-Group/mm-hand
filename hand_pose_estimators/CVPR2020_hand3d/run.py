...
if __name__ == '__main__':
    ...
    for img_name in image_list:
        ...
        for i, kp in enumerate(coord_hw_crop):
            kp = tuple(kp.astype(np.uint8)[::-1])
            image_crop_v = cv2.circle(image_crop_v, kp, 2, [255, 255, 255], 2)
            image_crop_v = cv2.putText(image_crop_v, str(i), kp,
                                       cv2.FONT_HERSHEY_SIMPLEX, 1, color=[0, 0, 255], thickness=2)

        import pdb; pdb.set_trace()
        cv2.imwrite(img_name.replace(".png", "_pred.png"), image_crop_v)
