...
class BinaryDbReaderSTB(object):
    """
        Reads data from a binary dataset created by create_STB_DB.py
    """
    ...
    def __init__(self, mode=None, batch_size=1, shuffle=True, use_wrist_coord=True, sigma=25.0, hand_crop=False,
        ...
        self.num_samples = 0
        if mode == 'training':
            # self.path_to_db += 'stb_training.bin'
            # self.num_samples = 60000
            self.path_to_db += 'stb_training.bin'
            self.num_samples = 60000
        elif mode == 'evaluation':
            self.path_to_db += 'stb_evaluate.bin'
            self.num_samples = 12000
        elif mode == 'training_gan':
            self.path_to_db = './data/bin/gan_evaluate.bin'#'./data/bin/gan_train.bin'
            self.num_samples = 29696#113753
        elif mode == 'evaluation_gan':
            self.path_to_db = './data/bin/gan_evaluate.bin'
            self.num_samples = 29696
        else:
        ...
        self.scale_target_size = (256, 256)  # size its scaled down to if scale_to_size=True
    ...
    def get(self):
        ...
        data_dict['hand_side'] = tf.one_hot(tf.constant(0, dtype=tf.int32), depth=2, on_value=1.0, off_value=0.0,
                                            dtype=tf.float32)
        ...
        index_root_bone_length = tf.sqrt(
            tf.reduce_sum(tf.square(kp_coord_xyz21_rel[12, :] - kp_coord_xyz21_rel[11, :])))
    ...
    def convert_kp(keypoints):
        ...
        # kp_dict = {0: 0, 1: 20, 2: 19, 3: 18, 4: 17,  # check
        #            5: 16, 6: 15, 7: 14, 8: 13,
