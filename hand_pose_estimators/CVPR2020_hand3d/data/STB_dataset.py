...
class STBdataset():
    def __init__(self, opt):
        ...
        :param kwargs:
        """
        super(STBdataset, self).__init__()
    ...
    def __getitem__(self, item):
        if self.opt.isTrain:
            ...
            return self.get_everything(item)

    def get_everything(self, item):
        image_path = self.image_color_SK[item]
        image_labels = self.get_labels(image_path)
        image = cv2.cvtColor(cv2.imread(image_path), cv2.COLOR_BGR2RGB) / 255.0 - 0.5
        keypoint_xyz21 = image_labels['xyz'] / 1000 # convert to meter
    ...
    def _get_item_vox2vox(self, item):
