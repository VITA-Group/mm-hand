...
class PosePriorNetwork(object):
    ...
    def inference(self, scoremap, hand_side, evaluation):
        ...
        if self.variant == 'direct':
            coord_xyz_rel_normed = self._inference_pose3d_can(scoremap_pooled, hand_side, evaluation, train=True)
            coord3d = coord_xyz_rel_normed
        elif self.variant == 'bottleneck':
            coord_xyz_rel_normed = self._inference_pose3d_can(scoremap_pooled, hand_side, evaluation, train=True, bottleneck=True)
            coord3d = coord_xyz_rel_normed
        elif (self.variant == 'local') or (self.variant == 'local_w_xyz_loss'):
            coord_xyz_rel_loc = self._inference_pose3d_can(scoremap_pooled, hand_side, evaluation, train=True)
            ...
            coord_can = self._inference_pose3d_can(scoremap_pooled, hand_side, evaluation, train=True)
            coord3d = coord_can
    ...
    def _inference_pose3d_can(self, keypoints_scoremap, hand_side, evaluation, train=False, bottleneck=False):
