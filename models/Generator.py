...
class PATBlock(nn.Module):
    def __init__(self,
        ...
        self.conv_block_stream1 = self.build_conv_block(dim,
                                                        padding_type,
                                                        norm_layer,
                                                        use_dropout,
                                                        use_bias,
        ...
        self.conv_block_stream3 = self.build_conv_block(
            dim,
            padding_type,
            norm_layer,
            use_dropout,
    ...
    def forward(self, x1, x2, x3):
        ...
        x3_out = self.conv_block_stream3(x3)

        att_pose = F.sigmoid(x2_out)
        att_depth = F.sigmoid(x3_out)
        '''Added'''
        x1_out = x1_out * att_pose * att_depth
        # x1_out = x1_out * att_pose
        out = x1 + x1_out  # residual connection

        # stream2 receive feedback from stream1
        x2_out = torch.cat((x2_out, out), 1)
        x3_out = torch.cat((x3_out, out), 1)
        return out, x3_out, x2_out, x1_out
...
class PATNModel(nn.Module):
    def __init__(self,
        ...
        self.stream3_down = nn.Sequential(*model_stream3_down)
    ...
    def forward(self, input):  # x from stream 1 and stream 2
        ...
        x3 = self.stream3_down(x3)
        # att_block
        for model in self.att:
            x1, x2, x3, _ = model(x1, x2, x3)
...
class Generator(nn.Module):
    def __init__(self,
        ...
        assert type(input_nc) == list and len(
            input_nc
        ) == 3, 'The AttModule take input_nc in format of list only!!'
        self.gpu_ids = gpu_ids
        self.model = PATNModel(input_nc,
                               output_nc,
                               ngf,
                               norm_layer,
                               use_dropout,
