...
class L1_plus_perceptualLoss(nn.Module):
    def __init__(self, lambda_L1, lambda_perceptual, perceptual_layers,
        ...
        self.vgg_submodel = torch.nn.DataParallel(self.vgg_submodel,
                                                  device_ids=gpu_ids).to(
                                                      self.gpu_ids[0])

    def forward(self, inputs, targets):
        if self.lambda_L1 == 0 and self.lambda_perceptual == 0:
            return Variable(torch.zeros(1)).to(self.gpu_ids[0]), Variable(
                torch.zeros(1)), Variable(torch.zeros(1))
        # normal L1
        ...
        mean[2] = 0.406
        mean = Variable(mean)
        mean = mean.resize(1, 3, 1, 1).to(self.gpu_ids[0])
        ...
        std[2] = 0.225
        std = Variable(std)
        std = std.resize(1, 3, 1, 1).to(self.gpu_ids[0])
        ...
        if self.percep_is_l1 == 1:
            ...
            loss_perceptual = F.l1_loss(
                fake_p2_norm, input_p2_norm_no_grad) * self.lambda_perceptual
        else:
            # use l2 for perceptual loss
            loss_perceptual = F.mse_loss(
                fake_p2_norm, input_p2_norm_no_grad) * self.lambda_perceptual
